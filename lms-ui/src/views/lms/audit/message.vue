            ...
            >
                <template #default="scope">
                    <el-button
                        ...
                        type="primary"
                        icon="Check"
                        @click="auditStore.passAudit(scope.row.id)"
                        >通过</el-button
                    >
                    <el-button
                        size="small"
                        type="danger"
                        icon="Close"
                        @click="auditStore.refuseAudit(scope.row.id)"
                        >拒绝</el-button
                    >
                </template>
            </el-table-column>
...
export default {
    ...
    setup() {
        ...
        return { userStore, auditStore };
    },
    data() {
        return {
            userId: this.userStore.userId,
            recordList: this.auditStore.recordList,
            // 遮罩层
            loading: false,
            // 总条数
            total: this.auditStore.recordList.length,
            // 查询参数
            queryParams: {
                pageNum: 1,
                pageSize: 10,
    ...
    methods: {
        ...
        getList() {
            this.loading = true;
            listRecordByAuditorId(this.userId).then((response) => {
